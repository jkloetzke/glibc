...
{
  int err;
  unsigned int * futex_word, expected;
  ...
  if (monotonic)
	...
	err = futex_reltimed_wait_cancelable (futex_word, expected, &rt,
					      sem->private);
    }
  else
    err = futex_abstimed_wait_cancelable (futex_word, expected, abstime,
					  sem->private);
